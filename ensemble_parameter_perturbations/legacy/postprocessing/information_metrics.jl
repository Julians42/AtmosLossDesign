...
# Run the example only when this file is executed directly, not when it's
# `include`d by resolution_effect.jl for its `analyze_variable_information`/
# `EffectiveRankMetric` definitions.
