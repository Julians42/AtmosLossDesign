...
function CAL.observation_map(
    ...
    config_dict = YAML.load_file(
        joinpath(dirname(Base.active_project()), "config", "experiment_config.yml"),
    ),
)
    # `output_dir` may still be a project-relative path (e.g. "data/output_5_cfsites")
    # if `config_dict` was loaded fresh above rather than via `resolve_config_paths!`.
    if !isabspath(config_dict["output_dir"])
        config_dict["output_dir"] = joinpath(dirname(Base.active_project()), config_dict["output_dir"])
    end
