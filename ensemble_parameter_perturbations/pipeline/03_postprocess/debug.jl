# Interactive debugging helper for troubleshooting a single (member, site) pair
# during postprocessing, without re-running the full `multi_postproc.jl` loop.
#
# Not a standalone script: run it from a REPL that has already `include`d
# src/methods.jl, src/parameter_io.jl, src/experiment.jl (for
# `normalize_parameters`) and loaded `config`/`param_stats_df`, e.g.:
#
#   include("pipeline/03_postprocess/debug.jl")
#   test_process_member_site("member_001", "<site_dir_name>", param_stats_df, config)
