# Compute the parameter informedness for several different observation
# scenarios (dropping advanced cloud variables, adding TKE/entrainment/updraft
# area fraction, varying resolution) and compare them against the full
# high-resolution, all-variable case.
#
# This is the merged form of what used to be two byte-identical files
# (realistic_marginals.jl and agu_realistic_marginals.jl) - if you need an
# AGU-specific variant again, add a new scenario/config below rather than
# forking the whole script.
...
with_theme(theme_minimal(), fontsize = 20) do
    ...
    save(joinpath(PROJECT_ROOT, "plots", "experiment_parameter_informedness.png"), fig)
end
